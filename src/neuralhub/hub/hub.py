...
class AgentHub:
    ...
    async def get_agent_status(self, agent_id: str) -> Dict[str, Any]:
        """Return status for a specific registered agent (for workflow wait-on-agent + NeuralCore engine compatibility)."""
        agent = self.agents.get(agent_id)
        if agent is None:
            return {"agent_id": agent_id, "status": "unknown", "error": "not registered with hub"}
