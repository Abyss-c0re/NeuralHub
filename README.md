...
`neuralhub` lets you run swarms of agents that communicate either **locally** (same process, using direct core methods + `TaskExecutor` task splitting) or over WebSocket.
...
|-----------------------------|----------------------------------------------------|--------------------------------------------------|----------|
| **Pure Local**              | `AgentHub(enable_central_hub=False, enable_agent_bridges=False)` or plain `NeuralHub()` | Direct `Agent` objects + `TaskExecutor.plan()` / `dispatch_parallel()` | Same-process swarms, maximum speed, no sockets |
...
- **Local-only cooperation** (new): `delegate_local_task()`, `orchestrate_local_split()`, `get_local_agent()` — uses the agent's native `request_agent()` / `TaskExecutor` primitives
...
# High-level local cooperation using NeuralCore primitives + TaskExecutor task splitting
...
`get_local_agent("id")` returns the live `neuralcore.Agent` object so you can call `request_agent()`, construct a `TaskExecutor(agent)`, etc. directly when you need fine-grained control.

See `tests/test_multi_agent_hub.py` for WebSocket examples and the NeuralCore cooperation tests (`test_agent_cooperation.py`) for the underlying `request_agent` / `TaskExecutor` patterns.
...
- Task decomposition and parallel dispatch are performed by `TaskExecutor.plan()` + `dispatch_parallel()`.
